...
@section('breadcumb')
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('trusty.users.index') }}">Roles</a></li>
    </ol>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="text-center">Kelola Data Users</h5>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="text-right mb-3">
                        <a href="{{ route('trusty.users.create') }}" class="btn btn-primary"><em
                                class="fa fa-plus"></em>
                            New User</a>
                    </div>
                </div>
                    ...
                    <table class="table table-bordered table-striped table-hover" id="users-table">
                        ...
                        <tbody>
                            @foreach ($users as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td><a href="{{ route('trusty.users.show', $item) }}">{{ $item->name }}</a>
                                    </td>
                                    <td>{{ $item->email }}</td>
                                </tr>
                            @endforeach
